...
class PurchaseRequest extends AbstractRequest
{
    ...
    public function getData()
    {
        ...
        $data['order_number']     = Str::limit($pars['transactionId'], 30);
        $data['language']         = $pars['language'];
        $data['currency']         = $pars['currency'];
        $data['amount']           = $pars['amount'];
        $data['cart']             = Str::limit($pars['description'], 255);
        ...
        if (!empty($pars['cardholderName'])) {
            $data['cardholder_name']     = Str::limit($pars['cardholderName'], 40);
        }
        if (!empty($pars['cardholderSurname'])) {
            $data['cardholder_surname']  = Str::limit($pars['cardholderSurname'], 40);
        }
        if (!empty($pars['cardholderAddress'])) {
            $data['cardholder_address']  = Str::limit($pars['cardholderAddress'], 40);
        }
        if (!empty($pars['cardholderCity'])) {
            $data['cardholder_city']     = Str::limit($pars['cardholderCity'], 20);
        }
        if (!empty($pars['cardholderZipCode'])) {
            $data['cardholder_zip_code'] = Str::limit($pars['cardholderZipCode'], 9);
        }
        if (!empty($pars['cardholderCountry'])) {
            $data['cardholder_country']  = Str::limit($pars['cardholderCountry'], 30);
        }
        if (!empty($pars['cardholderPhone'])) {
            $data['cardholder_phone']    = Str::limit($pars['cardholderPhone'], 30);
        }
        if (!empty($pars['cardholderEmail'])) {
            $data['cardholder_email']    = Str::limit($pars['cardholderEmail'], 40);
        }
