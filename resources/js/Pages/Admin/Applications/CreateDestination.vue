<template>
  <dialog-modal :show="showModal" @close="$emit('close')">
    ...
    <template #footer>
      <button
        type="button"
        @click.native="submitDestination"
...
export default {
  ...
  methods: {
    submitDestination() {
      const url = this.destinationId
        ? route("admin.destinations.update", this.destinationId)
        : route("admin.destinations.store");
      const method = this.destinationId ? { _method: "PUT" } : {};
      this.form
        .transform((data) => ({
          ...method,
          ...data,
